...
The app discovers saved adapter directories under `outputs/`. Select one, choose a device, click **Load model**, then provide a question and denoising settings. It uses the selected run's base model and tokenizer, its LoRA adapter, saved normalization parameters, and the same bidirectional attention mask as training. During generation it streams the intermediate states: confidence-colored tokens, gray `MASK` positions, blue permanently retained tokens, and the current denoising step. The optional copyable inference trajectory records both the complete prediction before each re-mask operation and the resulting masked state, including explicit EOS and native EOT markers throughout the full token block. A block length smaller than the answer length enables LLaDA-style semi-autoregressive generation: each block is fully denoised and then used as fixed context for the next block, while the selected total denoising-step budget is distributed across blocks. A block length equal to or larger than the answer length retains full-sequence denoising. Inference includes temperature, top-k, a tunable token repetition penalty, permanent unmasking, confidence-guided retention, and proportional unmasking. A repetition penalty of `1.0` is disabled; predicting occurrence N divides that token's unnormalized probability weight by `repetition_penalty ** (N - 1)`, so repeated answer tokens are discouraged progressively more strongly. The separate EOS/EOT prediction penalty also uses `1.0` as off; larger values divide only the EOS and native end-of-turn tokens' sampling weights and do not affect which positions are retained or re-masked. The LLaDA-style delayed EOS/EOT checkbox instead gives sampled end tokens the lowest retention confidence and therefore enables confidence-guided position selection.

## Benchmark validation

The optional benchmark runner evaluates every saved `best/` adapter, or the model paths listed in `configs/benchmarks.yaml`, using pure diffusion generation. Set the top-level `diffusion_sampler` to `llada_official` or `denoise_stream` to use that sampler for every selected model family, including hosted LLaDA, current mask-only adapters, and structured/legacy checkpoints. Without the global switch, structured runs retain LAD-style denoise-stream defaults while mask-only and hosted LLaDA runs retain the official fixed-budget sampler. Autoregressive evaluation of the original base model is optional and disabled by default.
